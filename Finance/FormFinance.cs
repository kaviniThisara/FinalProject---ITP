...
namespace Finance
{
    public partial class FormFinance : Form
    {
        ...
        private void inAccSave_Click(object sender, EventArgs e)
        {
            ...
            {
                MySqlConnection con = new MySqlConnection("server=localhost;user id=root;database=financedepartment");
                con.Open();
        ...
        private void empBtnSave_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                MySqlConnection con = new MySqlConnection("server=localhost;user id=root;database=financedepartment");
                con.Open();
                MySqlCommand cmd = new MySqlCommand("INSERT INTO `employee`(`emp_ID`, `emp_slryStrtDte`, `emp_slryEndDte`, `emp_totSlry`) VALUES ('" + empID.Text + "', '" + empSlryStrtDte.Value.ToShortDateString() + "', '" + empSlryEndDte.Value.ToShortDateString() + "', '" + empTotSal.Text + "')", con);
                cmd.ExecuteNonQuery();
        ...
        private void empBtnUpdate_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                MySqlConnection con = new MySqlConnection("server=localhost;user id=root;database=financedepartment");
                con.Open();
                MySqlCommand cmd = new MySqlCommand("UPDATE `employee` SET `emp_ID`= '" + empID.Text + "',`emp_slryStrtDte`= '" + empSlryStrtDte.Value.ToShortDateString() + "',`emp_slryEndDte`= '" + empSlryEndDte.Value.ToShortDateString() + "',`emp_totSlry`= '" + empTotSal.Text + "' WHERE emp_ID = '" + empID.Text + "'", con);
                cmd.ExecuteNonQuery();
                MessageBox.Show("Successfully Updated");
                con.Close();
        ...
        private void empBtnDelete_Click(object sender, EventArgs e)
        {
            ...
            MySqlConnection con = new MySqlConnection("server=localhost;user id=root;database=financedepartment");
            con.Open();
            MySqlCommand cmd = new MySqlCommand("DELETE FROM `employee` WHERE emp_ID = '" + empID.Text + "'", con);
            cmd.ExecuteNonQuery();
            MessageBox.Show("successfully deleted");
            con.Close();
        }
        ...
        void gridViewEmployee()
        {
            MySqlConnection con = new MySqlConnection("server=localhost;user id=root;database=financedepartment");
            con.Open();
            MySqlDataAdapter sda = new MySqlDataAdapter("select * from employee", con);
            DataTable dt = new DataTable();
            sda.Fill(dt);
            foreach (DataRow item in dt.Rows)
            {
